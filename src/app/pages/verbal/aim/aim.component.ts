...
export class AimComponent {
  ...
  moveTarget() {
    const top = Math.random() * (window.innerHeight / 2 - 100); // 100 being the size of the div
    const left = Math.random() * (window.innerWidth - 100); // 100 being the size of the div
    this.targetStyle = {
      'position': 'absolute',
      'top': `${top}px`,
      'left': `${left}px`
